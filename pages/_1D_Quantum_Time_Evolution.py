...
with st.sidebar:
    ...
    with c1:
        ...
        initial_wave_packet: WavePacket = st.selectbox("Packet $\\psi_0(x)$", [psi.value for psi in WavePacket])
        match initial_wave_packet:
            case WavePacket.Gaussian.value:
                st.latex("\\psi(x) \\propto e^{{-d(x-x_0)^2 + ipx }}")
            ...
            case WavePacket.Step.value:
                st.latex("\\psi(x) \\propto \\mathbb{1}_{{[r,s]}}e^{{ipx}}")
    ...
    with c3:
        n = st.number_input("$N_x$", value=1000)
    c1, c2, c3 = st.columns(3)
    with c1:
        number_of_energy_levels = st.number_input("$E_{levels}$", min_value=2, value=5)
    with c2:
    ...
    valid_x = np.linspace(x_min, x_max, n)
...
with c1:
    ...
    st.caption(f"""We known ${100 * np.sum([np.abs(el.component) ** 2 for el in energy_levels]):.20g} \\%$ of $\\ket{{\\psi_0}}$""")
...
time_evolution_data = pd.DataFrame(
    data = np.array([
        ...
        for temp_t
        in tqdm(np.linspace(0, time, trotterization_steps+1))
    ]).T,
